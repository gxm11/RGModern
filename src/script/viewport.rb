...
class Viewport
  ...
  def initialize(x_or_rect, y = nil, width = nil, height = nil)
    ...
    @data_ptr = RGM::Base.viewport_create(self)
  ...
  def ox=(ox)
    ...
    if @ox != ox
      @ox = ox
      RGM::Base.viewport_refresh_value(@data_ptr, RGM::Word::Attribute_ox) unless @disposed
    end
    @ox
  end
  ...
  def oy=(oy)
    ...
    if @oy != oy
      @oy = oy
      RGM::Base.viewport_refresh_value(@data_ptr, RGM::Word::Attribute_oy) unless @disposed
    end
    @oy
  end
